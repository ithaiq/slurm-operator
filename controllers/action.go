...
func (o *PatchObject) Execute(ctx context.Context) (noChange bool, err error) {
	if reflect.DeepEqual(o.original, o.new) {
		return true, nil
	}
...
func (o *CreateObject) Execute(ctx context.Context) (noChange bool, err error) {
...
func (o *BatchCreateObject) Execute(ctx context.Context) (noChange bool, err error) {
...
func (o *BatchPatchObject) Execute(ctx context.Context) (noChange bool, err error) {
	noChange = true
	for key, original := range o.originals {
		...
		if err = o.client.Patch(ctx, o.news[key], client.MergeFrom(original)); err != nil {
			return noChange, fmt.Errorf("BatchPatchObject error %q", err)
		}
	}
	return noChange, nil
}
...
func (o *BatchCreateOrUpdateObject) Execute(ctx context.Context) (noChange bool, err error) {
	noChange = true
	for key, original := range o.originals {
		if reflect.DeepEqual(original, o.news[key]) {
			continue
		...
		noChange = false
		if o.news[key] == nil {
			return noChange, fmt.Errorf("BatchCreateOrUpdateObject news key %q is empty", key)
		}
		if reflect.ValueOf(original).IsNil() {
			if err = o.client.Create(ctx, o.news[key]); err != nil {
				return noChange, fmt.Errorf("BatchCreateOrUpdateObject create error %q", err)
			}
		} else {
			if err = o.client.Update(ctx, o.news[key]); err != nil {
				return noChange, fmt.Errorf("BatchCreateOrUpdateObject update error %q", err)
			}
		}
	}
	return noChange, nil
